...
static int msm_otg_start_srp(struct usb_otg *otg)
{
	 ...
	 */
	ulpi_write(otg->usb_phy, 0x03, 0x97);
	/*
...
static void msm_otg_bus_vote(struct msm_otg *motg, enum usb_bus_vote vote)
{
	...
	msm_otg_dbg_log_event(&motg->phy, "BUS VOTE", vote, motg->phy.otg->state);
...
skip_phy_resume:
	...
	if (motg->async_int) {
		...
		motg->async_int = 0;
		if (phy->otg->state >= OTG_STATE_A_IDLE)
			set_bit(A_BUS_REQ, &motg->inputs);
	}
	motg->ui_enabled = 1;
	enable_irq(motg->irq);
...
static void msm_otg_notify_host_mode(struct msm_otg *motg, bool host_mode)
{
	...
	if (legacy_power_supply) {
		/* legacy support */
		if (host_mode) {
			power_supply_set_property(psy, POWER_SUPPLY_PROP_SCOPE, POWER_SUPPLY_SCOPE_SYSTEM);
		} else {
			power_supply_set_property(psy, POWER_SUPPLY_PROP_SCOPE, POWER_SUPPLY_SCOPE_DEVICE);
			/*
...
static int msm_otg_notify_chg_type(struct msm_otg *motg)
{
	...
	power_supply_set_property(psy, POWER_SUPPLY_PROP_TYPE, charger_type);
	return 0;
}
...
static int msm_otg_notify_power_supply(struct msm_otg *motg, unsigned mA)
{
	...
	if (motg->cur_power == 0 && mA > 2) {
		/* Enable charging */
		if (power_supply_set_property(psy, POWER_SUPPLY_PROP_ONLINE, true))
			goto psy_error;
		if (power_supply_set_property(psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, 1000*mA))
			goto psy_error;
	} else if (motg->cur_power >= 0 && (mA == 0 || mA == 2)) {
		/* Disable charging */
		if (power_supply_set_property(psy, POWER_SUPPLY_PROP_ONLINE, false))
			goto psy_error;
...
{
	// TODO: what's going on here? irq_read_line doesn't exist upstream
	return -ENODEV;
	/*unsigned long flags;
	int id;
...
static void msm_otg_mhl_notify_online(int on)
{
	...
	if (queue && phy->otg->state != OTG_STATE_UNDEFINED)
...
static void msm_otg_chg_check_timer_func(struct timer_list *t)
{
	struct msm_otg *motg = from_timer(motg, t, chg_check_timer);
	struct usb_otg *otg = motg->phy.otg;
...
static void msm_otg_id_timer_func(struct timer_list *t)
{
	...
	if (atomic_read(&motg->in_lpm)) {
		...
		msm_otg_dbg_log_event(&motg->phy, "ID TIMER: IN LPM",
				motg->phy.otg->state, 0);
		return;
	}

	if (motg->phy.otg->state == OTG_STATE_A_SUSPEND)
		goto out;
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	pm_runtime_resume(otg->usb_phy->dev);
	if (motg->pm_done) {
		msm_otg_dbg_log_event(&motg->phy, "PM RUNTIME: USBCONN GET",
			get_pm_runtime_counter(otg->usb_phy->dev), motg->pm_done);
		pm_runtime_get_sync(otg->usb_phy->dev);
		motg->pm_done = 0;
	}
	pr_debug("%s work\n", usb_otg_state_string(otg->state));
	...
	case OTG_STATE_UNDEFINED:
		...
		if (!test_bit(B_SESS_VLD, &motg->inputs) &&
			...
			pm_runtime_suspend(otg->usb_phy->dev);
			break;
		}
		/* FALL THROUGH */
		...
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_DCP_CHARGER:
					...
					break;
					//TODO: are these two important? OTG_STATE_B_CHARGER doesn't exist in upstream and I don't know what it means
/* 				case USB_PROPRIETARY_CHARGER:
					...
					get_pm_runtime_counter(otg->usb_phy->dev),
					0);
					pm_runtime_put_sync(otg->usb_phy->dev);
					break;
			...
			pm_runtime_autosuspend(otg->usb_phy->dev);
			motg->pm_done = 1;
			msm_otg_dbg_log_event(&motg->phy,
	...
	case OTG_STATE_B_SRP_INIT:
		if (!test_bit(ID, &motg->inputs) ||
			 ...
			 */
			ulpi_write(otg->usb_phy, 0x0, 0x98);
			work = 1;
	...
	case OTG_STATE_B_PERIPHERAL:
		if (test_bit(B_SESS_VLD, &motg->inputs) &&
			...
			motg->falsesdp_retry_count = 0;
			msm_otg_notify_charger(motg, 0);
			// TODO: this is probably important for OTG to work or something, but otg_srp_reqd doesn't exist upstream
		...
		break;
		// TODO: this might be important, but OTG_STATE_B_CHARGER doesn't exist in upstream?
	...
	if (atomic_read(&motg->in_lpm)) {
		...
		if (!atomic_read(&motg->pm_suspended)) {
			if (otg->state >= OTG_STATE_A_IDLE)
				set_bit(A_BUS_REQ, &motg->inputs);
			pm_request_resume(otg->usb_phy->dev);
		}
		return IRQ_HANDLED;
	}
...
static void msm_id_status_w(struct work_struct *w)
{
	...
	if (motg->id_state) {
		...
		queue_delayed_work(system_wq, &motg->id_status_work,
				  ...
				  const union power_supply_propval *val)
{
	...
	msm_otg_dbg_log_event(&motg->phy, "SET PWR PROPERTY", psp, psy->desc->type);
	switch (psp) {
...
/*	case POWER_SUPPLY_PROP_DP_DM:
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_TYPE:
		power_supply_set_property(psy, POWER_SUPPLY_PROP_TYPE, val->intval);
		...
		if (motg->chg_state == USB_CHG_STATE_DETECTED) {
			if (psy->desc->type == POWER_SUPPLY_TYPE_UNKNOWN)
				power_supply_set_property(psy, POWER_SUPPLY_PROP_TYPE, POWER_SUPPLY_TYPE_USB);
...
/* 		case POWER_SUPPLY_TYPE_USB_HVDCP:
		...
		msm_otg_dbg_log_event(&motg->phy, "SET CHARGER TYPE ",
				motg->chg_type, psy->desc->type);
		break;
	case POWER_SUPPLY_PROP_HEALTH:
		motg->usbin_health = val->intval;
		break;
		// TODO: is this important? POWER_SUPPLY_PROP_ALLOW_DETECTION doesn't exist upstream
					...
					struct msm_otg *motg)
{
	...
	ret = power_supply_register(&pdev->dev, &motg->usb_psy_desc, &motg->usb_psy_config);
	if (ret < 0) {
		dev_err(motg->phy.dev,
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	timer_setup(&motg->id_timer, msm_otg_id_timer_func,
				0);//(unsigned long) motg);
	timer_setup(&motg->chg_check_timer, msm_otg_chg_check_timer_func,
				0);//(unsigned long) motg);
	...
	motg->usb_psy.desc = &motg->usb_psy_desc;
	motg->usb_psy_desc.name = "usb";
	motg->usb_psy_desc.type = POWER_SUPPLY_TYPE_USB;
	motg->usb_psy_config.supplied_to = otg_pm_power_supplied_to;
	motg->usb_psy_config.num_supplicants = ARRAY_SIZE(otg_pm_power_supplied_to);
	motg->usb_psy_desc.properties = otg_pm_power_props_usb;
	motg->usb_psy_desc.num_properties = ARRAY_SIZE(otg_pm_power_props_usb);
	motg->usb_psy_desc.get_property = otg_power_get_property_usb;
	motg->usb_psy_desc.set_property = otg_power_set_property_usb;
	motg->usb_psy_desc.property_is_writeable
