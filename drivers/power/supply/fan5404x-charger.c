...
static int fan5404x_get_prop_batt_capacity(struct fan5404x_chg *chip)
{
	...
	if (chip->shutdown_voltage_tripped && !chip->factory_mode) {
		if ((chip->usb_psy) && chip->usb_present) {
			power_supply_set_property(chip->usb_psy, POWER_SUPPLY_PROP_PRESENT, false);
			power_supply_set_property(chip->usb_psy, POWER_SUPPLY_PROP_PRESENT, false);
...
static int fan5404x_get_prop_batt_health(struct fan5404x_chg *chip)
{
	...
	if (chip->batt_hot)
		...
		batt_health = POWER_SUPPLY_HEALTH_GOOD;//WARM;
	else if (chip->batt_cool)
		batt_health = POWER_SUPPLY_HEALTH_GOOD;//COOL;
	else
...
static int fan5404x_regulator_init(struct fan5404x_chg *chip)
{
	...
	init_data = of_get_regulator_init_data(chip->dev, chip->dev->of_node, desc);
	if (!init_data) {
				...
				const struct i2c_device_id *id)
{
	...
	chip->batt_psy_desc.set_property = fan5404x_batt_set_property;
	chip->batt_psy_desc.properties = fan5404x_batt_properties;
	chip->batt_psy_desc.num_properties = ARRAY_SIZE(fan5404x_batt_properties);
	chip->batt_psy_desc.external_power_changed = fan5404x_external_power_changed;
	chip->batt_psy_desc.property_is_writeable = fan5404x_batt_is_writeable;

	struct power_supply_config batt_cfg = {};

	rc = power_supply_register(chip->dev, &chip->batt_psy_desc, &batt_cfg);
	if (rc < 0) {
		dev_err(&client->dev,
...
#ifdef CONFIG_OF
static const struct of_device_id fan5404x_dt_match[] = {
	{ .compatible = "fairchild,fan54046-charger" },
	{ .compatible = "fairchild,fan5404x-charger" },
	{ },
};
MODULE_DEVICE_TABLE(of, fan5404x_dt_match);
#endif
...
static struct i2c_driver fan5404x_charger_driver = {
	.driver		= {
		...
		.owner		= THIS_MODULE,
		.of_match_table	= of_match_ptr(fan5404x_dt_match),
		.pm		= &fan5404x_pm_ops,
	},
